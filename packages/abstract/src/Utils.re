...
module String = {
  ...
  let replace = (~old, ~new_, str) =>
    Js.String.replaceByRe(
      Js.Re.fromStringWithFlags(old, ~flags="g"),
      new_,
      str,
    );

  let isMatch: (string, Js.Re.t) => bool =
    (s, r) => O_.isSome(Js.String.match(r, s));
  ...
  let dedupeList = (strings: list(string)): list(string) => {
    let (_, reversed) =
      Belt.List.reduce(strings, (SSet.empty, []), ((seen, uniques), s) =>
        SSet.has(seen, s)
          ? (seen, uniques)
          : (SSet.add(seen, s), Belt.List.add(uniques, s))
      );
...
module Array = {
  ...
  let pushMut = (arr: array('a), elem: 'a): unit =>
    Js.Array.push(elem, arr) |> ignore;
...
module Json = {
  ...
  let pretty: Js.Json.t => string = [%bs.raw
