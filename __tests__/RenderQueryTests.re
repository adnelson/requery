...
module SnapshotTests = {
  ...
  describe("createTable", () => {
    ...
    test("book", () =>
      expect(BooksExample.Book.createTable(QB.Types.int)->R.CreateTable.render)
      ->toMatchSnapshot()
    );
  });
  describe("select", () => {
    test("author books", () =>
      expect(BooksExample.authorBooksSelect->R.Select.render)->toMatchSnapshot()
    );
  ...
  describe("insert", () => {
    test("authors", () => {
      expect(R.Insert.render(~onConflict=string_of_int, BooksExample.insertAuthors))
      ->toMatchSnapshot()
    });
    test("authors with fake on conflict", () => {
      ...
      let rendered = R.Insert.render(~onConflict=renderOnConflict, withOnConflict);
